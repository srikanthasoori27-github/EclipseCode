 ...
 */
public class RoleCompositionEntityListFilterContext extends CertificationEntityListFilterContext {

    // Properties around roles
    private static final String CERT_ENTITY_ROLE_NAME = "targetName";
    private static final String CERT_ITEM_TYPE = "itemType";
    protected static final String CERT_ENTITY_FILTER_STRING_FORMAT = "certification.id == \"%s\"";

    /**
     * Constructor
     * @param certificationId ID of the certification
     */
    public RoleCompositionEntityListFilterContext(String certificationId) {
    ...
    public List<ListFilterDTO> getObjectAttributeFilters(SailPointContext context, Locale locale) throws GeneralException {
        ...
        ListFilterDTO relatedRoleNameFilter = new ListFilterDTO(
                CERT_ENTITY_ROLE_NAME, Internationalizer.getMessage(MessageKeys.UI_ROLE_COMP_CERT_NAME_COL, locale),
                false, ListFilterDTO.DataTypes.String, locale);
        relatedRoleNameFilter.configureColumnSuggest(
                CertificationEntity.class.getSimpleName(), CERT_ENTITY_ROLE_NAME, String.format(CERT_ENTITY_FILTER_STRING_FORMAT, getCertificationId()), getSuggestUrl());
