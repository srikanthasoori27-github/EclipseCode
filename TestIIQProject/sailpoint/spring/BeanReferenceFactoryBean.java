 ...
 * SailPoint Notes
 * 
 * jsl - this was copied over from the 1.2.6 sources because it was removed
 * somewhere along the way when we upgraded to 4.3.5.  <alias> is intended
 * as a replacement but I was not able to find a way to make that work with
 * iiq.properties file overrides like we do for <bean>s.  This is being used
 * to swap between the DBCP and JNDI data stores with a property in iiq.properties.
 * Supposedly <alias> supports "placeholder" references but it is unclear if that
 * can be combined with the use of iiq.properties with our use of
 * sailpoint.server.SpringPropertyOverrider.  If not then we have two properties
 * files which is almost as bad as just making deployers edit the .xml file.  
 * Since our use of Spring is so basic, just bring over the previous class
