/**
 * A utility for compiling a String of Java source and loading it as a Class.
 * This was taken from this IBM Developer Works example:
 *
 * http://www.ibm.com/developerworks/java/library/j-jcomp/
 *
 * Finder: Jeff
 * 
 * The source is nearly identical to the IBM site and contained no copyrights.
 *
 * The changes were:
 *
 *    - change the package to sailpoint.tools
 *    - make the castable check in compile() optional if tyoes is null
 *
 * Any changes will be prefixed with "jsl - "
 */
...
public class CharSequenceCompiler<T> {
   ...
   public synchronized Class<T> compile(final String qualifiedClassName,
      ...
      // jsl - made this conditional
      if (types != null)
          newClass = castable(newClass, types);
